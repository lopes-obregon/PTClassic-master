	...
	case CLANCHIP_SUBCHIP_LOAD_DATA:
		switch(C_menuN2){
			...
			DebugMessage("CLAN_CHIP_ReadDATA¿¡¼­ ¸®ÅÏÄÚµå 0 °¡ÀÔÇÑ Å¬·£ÀÌ ¾øÀ½");
				...
				default:
					if(ANSdata.getReturnValue() != -1) {		//ÀüÇô¾û¶×ÇÑ ´äÀÌ ¿Í¼­ ¿¡·¯°¡ ³µ´Ù.
					WebDB.Suspend();
...
//===============¼­¹ö°¡ Ã³¸®ÁßÀÏ¶§ ·Îµå ¸Þ´º º¸¿©ÁÜ
	case CLAN_REMOVE_SHORTCLANWON_LOADING:
		if(ANSdata.getReturnValue() !=-1) C_menuN = CLAN_REMOVE_SHORTCLANWON;
		break;
	case CLAN_REMOVE_LOADING:
		if(ANSdata.getReturnValue() !=-1) C_menuN = CLAN_REMOVE;
		break;
	case CLAN_RELEASE_LOADING: 
		if(ANSdata.getReturnValue() !=-1) C_menuN = CLAN_RECONF_RELEASE;
		break;	
	case CLAN_CHIP:
		i = chkeckMenuN();
	...
	case CLAN_REMOVE:
		...
		case 1:		
			...
			case 3: //Å»Åð´Â ¼º°øÇßÀ¸³ª Å¬·£È¸¿ø¼ö°¡ 6¸íÀÌÇÏ·Î ¶³¾îÁ³À» °æ¿ì(LimitDate ¼³Á¤)
				...
				menuInit(C_menuN);				
#ifdef CLAN_DEBUG
			DebugMessage("CLAN_REMOVE¿¡¼­ Ãß¹æ¼º°ø(%d)",ANSdata.getReturnValue());
#endif
			...
			case 2: //Å¬·£ÀÌ Á¸ÀçÇÏÁö ¾Ê°Å³ª Å¬·£ ÀåÀÌ ¾Æ´Ñ°æ¿ì ¶Ç´Â Å¬·£¿øÀÌ ¾Æ´Ñ°æ¿ì
				...
				C_menuN2=-1;
#ifdef CLAN_DEBUG
			DebugMessage("CLAN_REMOVE¿¡¼­ Å¬·£ÀÌ Á¸ÀçÇÏÁö ¾Ê°Å³ª Å¬·£ ÀåÀÌ ¾Æ´Ñ°æ¿ì ¶Ç´Â Å¬·£¿øÀÌ ¾Æ´Ñ°æ¿ì(%d)",ANSdata.getReturnValue());
#endif
				break;				
			case 100: //¾ûµÕÇÑ °ª
				WebDB.Suspend(); //À¥db ÁßÁö
