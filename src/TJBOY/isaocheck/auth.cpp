...
int WebDB_Thread::AUTH_proc_ReadNotice_Result( int quenum, int threadindex)
{
	...
	switch( iResultCode ){
	case 0:
		ANSdata.setReturnValue(iResultCode);				//tjclan.cpp¿¡ Á¤ÀÇµÈ º¯¼ö¿¡ ´äº¯À» ³Ö´Â´Ù.
		break;
	case 1:
		ANSdata.setReturnValue(iResultCode);				//tjclan.cpp¿¡ Á¤ÀÇµÈ º¯¼ö¿¡ ´äº¯À» ³Ö´Â´Ù.
		...
		break;	
	case 100:				//ÀÏ¹Ý ¸â¹ö,
		ANSdata.setReturnValue(iResultCode);
		break;
	case 1004:
		ANSdata.setReturnValue(iResultCode);
		break;
	default:
		ANSdata.setReturnValue(AUTH_ERROR_OTHER);	// ±âÅ¸ÀÇ ¿¡·¯
		break;
	}
	...
	*/
	//ANSdata.setReturnValue(iResultCode);				//tjclan.cpp¿¡ Á¤ÀÇµÈ º¯¼ö¿¡ ´äº¯À» ³Ö´Â´Ù.
	fd2("Å¬·£¸â¹ö¿©ºÎÀÇ ´äº¯À» ¹ÞÀ½. :  ret_val %d  ", ANSdata.getReturnValue());
...
int WebDB_Thread::ReadClanInfo(char *clanNum,CLANINFO *ci)
{
	...
	time_t myTime;
	time( ( time_t* )&myTime );
	AuthQueTmp.howmanyTime = (long)myTime;
...
int WebDB_Thread::AUTH_proc_isCheckClanJang_Result( int quenum, int threadindex)
{
	...
	switch( iResultCode ){
		...
		iResultCode = AUTH_que[quenum].result;	// µðÆúÆ®´Â ±âÅ¸ÀÇ ¿¡·¯
			break;
		case 1004:
		iResultCode = ANSdata.getReturnValue();
			break;
...
int WebDB_Thread::AUTH_proc_WarhouseR_Result( int quenum, int threadindex)
{
	...
	*/
	ANSdata.setReturnValue(iResultCode);				//tjclan.cpp¿¡ Á¤ÀÇµÈ º¯¼ö¿¡ ´äº¯À» ³Ö´Â´Ù.
	fd2("Ã¢°í±ÇÇÑ ´äº¯À» ¹ÞÀ½. :  ret_val %d  ", ANSdata.getReturnValue());
...
int WebDB_Thread::start_GOLDmoney(int fd, char *cdkey, char *accountid, char *passwd, char *ip_address)
{
	//return 0;
		...
		time_t myTime;
		time( ( time_t* )&myTime );
		AuthQueTmp.howmanyTime = (long)myTime;//Connect[fd].howmanyTime = myTime;
