...
int ISAOCHECK::isCheckClanJang(char *ClanZang_id, char* gserver)
{
	...
	//ÀÏº»Å¬·£
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/isCheckClanJang.php?userid=%s&gserver=%s&ticket=%d", hostName, hostPORT, ClanZang_id,gserver,nClanTicket);
...
int ISAOCHECK::isCheckClanName(char *ClanName,char *gserver)
{
	...
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/isCheckClanName.php?ClName=%s&gserver=%s&ticket=%d", hostName, hostPORT, ClanName,gserver,nClanTicket);
...
int ISAOCHECK::ReadClanInfo(char *clanNum,char* ret_Buf)
{
	...
	//ÀÏº»Å¬·£¹øÈ£
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/isCheckClanNum.php?num=%s&ticket=%d", hostName, hostPORT, clanNum,nClanTicket);
...
int ISAOCHECK::isCheckClanMember(char *pszServerName, char *pszUserName)
{
	...
	sprintf(szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/isCheckClanwon.php?gserver=%s&clwon=%s",hostName, hostPORT,
...
int ISAOCHECK::isClanMember(char *ClanZang_id, char* gserver,char* ClanZang_ChaName,char* ret_Buf)
{
	...
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/isClanMember.php?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, hostPORT, ClanZang_id,gserver,ClanZang_ChaName,nClanTicket);
...
//ÀÏº»Å¬·£»ý¼º	
	...
	(
	szWriteBuffer,
	"http://%s/Clan/clanInsert.php?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d",
	...
	sprintf
	(
	szWriteBuffer, "http://%s:%d/clanfiles/Clan/clanInsert.php?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d",
		hostName, hostPORT, ClanZang_ID, gserver,ClanZang_ChaName,ClanName,explanation,nClanTicket,chtype,level
	);
...
*/
	sprintf( szWriteBuffer,
		"http://%s:%d/clanfiles/ServerMain/GetAllMyClanMember.php?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, hostPORT, ClanZang_ID,gserver,ClanZang_ChaName,nClanTicket);
//#endif
...
*/
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/clanWonRelease.php?userid=%s&gserver=%s&chname=%s&clName=%s&%s&ticket=%d", hostName, hostPORT, ClanZang_ID,gserver,ClanZang_ChaName,ClanName,ClanWon,nClanTicket);
//#endif
...
int ISAOCHECK::AppointSubChip(char *gserver,char* clanwon)
{
	...
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/ClanSubChipUpdate.php?chname=%s&gserver=%s&ticket=%d", hostName, hostPORT, clanwon,gserver,9999);//nClanTicket);
...
int ISAOCHECK::ReleaseSubChip(char *gserver,char* clanwon)
{
	...
	sprintf( szWriteBuffer,
		"http://%s:%d/clanfiles/ServerMain/ClanSubChipRelease.php?chname=%s&gserver=%s&ticket=%d", hostName, hostPORT, clanwon,gserver,9999);//nClanTicket);
...
int ISAOCHECK::LeaveClan(char *clanname,char *chaname,char *gserver)
{
	...
	sprintf( szWriteBuffer,
		"http://%s:%d/clanfiles/ServerMain/clanChipChange.php?chname=%s&gserver=%s&clName=%s&ticket=%d", hostName, hostPORT, chaname,gserver,clanname,nClanTicket);
...
int ISAOCHECK::read_ClanImage(char *clanname,char *filename)
{
	...
	//ÃÖ±Ù °íÄ£°Í
	sprintf( szWriteBuffer, "http://%s:%d/clanfiles/ClanContent/%s.bmp", hostName, hostPORT, filename);
...
*/
	sprintf( szWriteBuffer,
	"http://%s:%d/clanfiles/ServerMain/sod2info.php?userid=%s&chname=%s&gserver=%s&index=%d", hostName, hostPORT, usid, charname, gserver, index);
//#endif
